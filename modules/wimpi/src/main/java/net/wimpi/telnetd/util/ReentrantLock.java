...
public class ReentrantLock {
  ...
  private static final Logger log = Logger.getLogger(ReentrantLock.class.getName());
  protected Thread m_Owner = null;
  protected long m_Holds = 0;

  public void acquire() throws InterruptedException {
    //log.fine("acquire()::" + Thread.currentThread().toString());
  ...
  public boolean attempt(long msecs) throws InterruptedException {
    //log.fine("attempt()::" + Thread.currentThread().toString());
    if (Thread.interrupted()) throw new InterruptedException();
    Thread caller = Thread.currentThread();
    synchronized (this) {
      if (caller == m_Owner) {
  ...
  public synchronized void release() {
    //log.fine("release()::" + Thread.currentThread().toString());
